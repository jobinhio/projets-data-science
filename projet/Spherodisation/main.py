...
def calcul_longueur_fil_et_limite_four(
    ...
    # K = (pct_mg_fonte_four_max - pct_mg_four_limite*(1 -pct_perdu_mg_coulee_min*temps_traitement ) ) / (2 -  pct_perdu_mg_poche_min*temps_gs)
    K = (pct_mg_fonte_coulee_max - pct_mg_four_limite ) / (2 -  pct_perdu_mg_poche_min*temps_gs)

    # Masse de Mg à ajouter dans la poche de traitement pour obtenir le Mg maximal
    pct_mg_fil = masse_mg_fil/masse_fil *100
    Q = calcul_quantite_mg(masse_fonte_poche,pct_Soufre,temps_gs ,tempera_fonte_poche,pct_rendement_mg,pct_mg_fil,K) # en Kg

    # Longueur de fil pour avoir la masse de Mg manquante
    longueur_fil_necessaire = Q / (masse_mg_fil* 1e-3)   # en m
...
def main_fct(chemin_fichier, dossier_courant):
    ...
    output_name = [
        ...
        'Temps restant avant le lancement du prochain traitement (en min)',
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python3 main.py Sphérodisation.xlsm")
        sys.exit(1)

...
def MakeFragment(_id , _id_branches):
    ...
    for e in zip([(2, _id)] + [(2, _id_branches)], ovv):
...
def MakeMesh(args,_id,_ids_Gamma_ext,lc,hExt):    
    ## pour raffiner le maillage au bord GammaExt
    if args.raffiner:
        ...
        gmsh.model.mesh.field.setNumber(2, "SizeMin", hExt) 
...
def main():
    ...
    parser.add_argument("--t", help="largeur d'un fin", type=float, default=0.25)
    parser.add_argument("--d", help="distance entre les fins", type=float, default=0.75)

    parser.add_argument("--hExt", help="taille mesh sur Gamma_ext", type=float, default=0.01)
    parser.add_argument("--raffiner", help="activer le mode raffiner de Gamma_ext", action="store_true")
    ...
    ## Maillage
    if args.mesh:
        MakeMesh(args,_id,_ids_Gamma_ext,lc,hExt)
